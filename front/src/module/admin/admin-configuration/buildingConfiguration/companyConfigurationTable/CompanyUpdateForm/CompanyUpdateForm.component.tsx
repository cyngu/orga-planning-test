...
const CompanyUpdateForm: React.FC = () => {
    ...
    const onSubmit: SubmitHandler<CompanyFormType> = async data => {
        await updateCompany({
            ...
            name: companyName ? companyName : companySelected.name,
            description: companyDescription ? companyDescription : companySelected.description,
            officeNumber: companyOfficeNumber ? companyOfficeNumber : companySelected.officeNumber,
        });
        getAllCompany();
    };

    useEffect(() => {
        setCompanyName(companySelected.name);
        setCompanyDescription(companySelected.description);
        setCompanyOfficeNumber(companySelected.officeNumber);
    }, [companySelected]);
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                <div className="flex flex-col justify-center items-center gap-3 text-sm">
                    <div className="w-[90%] border bg-white shadow-lg rounded-lg p-5">
                        <h2 className="text-xl font-bold pb-10 pt-2">Modification de l&apos;entreprise {companySelected.name}</h2>

...
const ParkingPlaceUpdateForm: React.FC = () => {
    ...
    const { isLoading, parkingPlaceSelected, updateParkingPlace, getAllParkingPlace } = useParkingPlaceStore()
    const {
        handleSubmit,
        formState: { errors },
    ...
    const onSubmit: SubmitHandler<ParkingPlaceFormType> = async data => {
        await updateParkingPlace({
            id: parkingPlaceSelected.id,
            name: parkingPlaceName ? parkingPlaceName : parkingPlaceSelected.name,
            chargingStation: isChargingStation ? isChargingStation : parkingPlaceSelected.chargingStation,
            underground: isUnderground ? isUnderground : parkingPlaceSelected.underground,
            entrepriseId: companyId ? companyId : parkingPlaceSelected.entrepriseId,
    ...
    useEffect(() => {
        getAllCompany();
        setParkingPlaceName(parkingPlaceSelected.name);
        setIsChargingStation(parkingPlaceSelected.chargingStation);
        setIsUnderground(parkingPlaceSelected.underground);
        setCompanyId(parkingPlaceSelected.entrepriseId);
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                <div className="flex flex-col justify-center items-center gap-3 text-sm">
                    <div className="w-[90%] border bg-white shadow-lg rounded-lg p-5">
                        <h2 className="text-xl font-bold pb-10 pt-2">Modification de la place numéro {parkingPlaceSelected.name}</h2>
...
export default ParkingPlaceUpdateForm;

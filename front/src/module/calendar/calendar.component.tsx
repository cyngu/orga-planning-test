'use client';
import { format, getDay, intlFormat, isWeekend, Locale, parse, startOfWeek } from 'date-fns';
...
import CustomToolbar from '~/components/calendar/calendarToolBar.component';
...
export const CalendarSample: React.FC = () => {
    ...
    const { setEvents, setEventId, setChoiceEventModal, setCreateEventModal, setDate, date, events, isLoading } = useCalendarStore();
    ...
    const onClickEmptyEvent = (event: { start: Date; end: Date }) => {
        ...
        if (!isCantOpen && !isWeekend(new Date(event.start))) {
